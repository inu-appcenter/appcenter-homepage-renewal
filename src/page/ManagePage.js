...
export default function ManagePage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
          isLoading(true);
      const viewData = await axios.get('https://server.inuappcenter.kr/members/all-members').then(res => {
          isLoading(false);
          setData(res.data);
        });
    }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      await axios.delete(`https://server.inuappcenter.kr/members/${selectedMemberId}`);
  ...
  /* useEffect(() => {
    ...
    <>
      ...
      <IntroBox>
        ...
        <Text type='top'>{'동아리원 추가, 삭제, 수정을 할 수 있어요'}</Text>
      </IntroBox>
        <MemberList>
            동아리원 목록
        </MemberList>
      ...
      <AddList>
        ...
        <AddMember
          ...
          onChange={(e) =>
            setNewMember({ ...newMember, gitRepositoryLink: e.target.value })
          }
        />
