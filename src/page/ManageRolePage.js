...
export default function ManageRolePage() {
  ...
  const addData = async () => {
    try {
      const result = await axios.post('https://server.inuappcenter.kr/roles', 
        newRole
      );
  ...
  useEffect(() => {
    const fetchData = async () => {
      const viewData = await axios.get('https://server.inuappcenter.kr/roles/all-roles').then(res => {
          setData(res.data);
        });
    }
    fetchData();
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      setData((prevData) =>
        prevData.map((item) =>
          item.member_id === selectedRoleId ? { ...item, ...updatedData } : item
        )
      );
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      await axios.delete(`https://server.inuappcenter.kr/roles/${selectedRoleId} `);
      console.log("Member with ID", selectedRoleId, "has been deleted.");
  
      // 삭제한 데이터를 data 상태에서 제거합니다.
      setData((prevData) => prevData.filter((item) => item.role_id !== selectedRoleId));
    } catch (error) {
